...
`
    ...
    const zh = {
      'title': '输入框皮肤',
      'hint': '输入框小剧场:内部双光束扩散巡视+边框线段圆角巡游。调整颜色/速度请改 lib/client.js 顶部「★ 调参区 ★」,刷新即生效。'
    }
    const en = {
      'title': 'Input skin',
      'hint': 'Composer mini-theater: inner beams spread + border light segments cruise on rounded arcs. Tune colors/speeds in the "★ 调参区 ★" block at the top of lib/client.js, then refresh.'
    }
    ...
    const PARAM_DEFS = [
      { key: 'beamL', label: '左束颜色', type: 'color' },
      { key: 'beamR', label: '右束颜色', type: 'color' },
      { key: 'beamL2', label: '第二组·左色', type: 'color' },
      { key: 'beamR2', label: '第二组·右色', type: 'color' },
      { key: 'segC1', label: '线段色①', type: 'color' },
      { key: 'segC2', label: '线段色②', type: 'color' },
      { key: 'segC3', label: '线段色③', type: 'color' },
      { key: 'beamFlow', label: '光束来回(秒)', type: 'range', min: 2, max: 12, step: 0.5, unit: 's' },
      { key: 'beamCycle', label: '配色周期(秒)', type: 'range', min: 8, max: 60, step: 2, unit: 's' },
      { key: 'beamWidth', label: '光束宽度(%)', type: 'range', min: 20, max: 80, step: 5, unit: '%' },
      { key: 'segColor', label: '线色循环(秒)', type: 'range', min: 1, max: 12, step: 0.5, unit: 's' },
      { key: 'segLap', label: '线段一圈(秒)', type: 'range', min: 6, max: 40, step: 1, unit: 's' },
      { key: 'segLen', label: '线段长度(px)', type: 'range', min: 20, max: 90, step: 2, unit: 'px' },
      { key: 'breathe', label: '呼吸(秒)', type: 'range', min: 0.8, max: 6, step: 0.2, unit: 's' },
    ]
    ...
    function applyParamValue(key, value) { try { document.body.style.setProperty(PARAM_VARS[key], String(value)) } catch {} }
    ...
    function ParamsPanel() {
      ...
      return h('div', { style: { display: 'flex', flexDirection: 'column', gap: '8px' } },
        h('button', { type: 'button', onClick: () => setOpen(!open), style: { ...styles.collapseBtn, ...(open ? styles.collapseBtnOpen : {}) } },
          (open ? '▾ ' : '▸ ') + '调整参数（颜色 / 速度 / 长度）'),
        open && h('div', { style: { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '6px 14px' } },
          PARAM_DEFS.map((def) => h(ParamControl, { key: def.key, def, value: params[def.key], onChange: (v) => change(def.key, v) })),
